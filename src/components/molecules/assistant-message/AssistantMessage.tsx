...
export function AssistantMessage({
  ...
  const defaultLabels = MESSAGE_LABELS[locale];

  const isAssistant = message.role === "assistant";
  const isSending = message.status === "sending";
  const isError = message.status === "error";

  const resolvedAuthorLabel = isAssistant
    ? assistantLabel ?? defaultLabels.assistant
    : userLabel ?? defaultLabels.user;

  const resolvedSourcesLabel =
    sourcesLabel ?? defaultLabels.sources;

  const resolvedSendingLabel =
    sendingLabel ?? defaultLabels.sending;
      ...
      >
        ...
        >
          ...
          {isSending ? (
            ...
            >
              ...
              <span>{resolvedSendingLabel}</span>
            </div>
          ) : null}
        </div>
        ...
        {showTimestamp && formattedTime ? (
          <time
            ...
            className={cn(
              "mt-1.5 inline-flex items-center gap-1 px-1",
