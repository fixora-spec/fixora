...
const VERIFICATION_BODY_LIMIT_BYTES =
  Math.min(
    AUTH_REQUEST_LIMITS
...
function getVerificationRateLimitIdentifier(
  request:
    Request,
...
export async function POST(
  ...
  let input:
    ReturnType<
      typeof validateEmailVerificationRequest
    >;
  ...
  const rateLimitIdentifier =
    getVerificationRateLimitIdentifier(
      request,
      input.accountId,
    );
  ...
  try {
    ...
    try {
      ...
      /*
       * Una verificación válida no debe fallar
       * solamente porque no se limpió el límite.
       */
    }

    return createApiSuccessResponse({
      account:
        serializeVerifiedAccount(
          result.account,
        ),
    });
  } catch (error) {
